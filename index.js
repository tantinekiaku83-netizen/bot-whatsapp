...
async function start() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        // Pede o código de emparelhamento se ainda não estiver registado
        if (!sock.authState.creds.registered) {
            setTimeout(async () => {
                try {
                    console.log("A solicitar novo código de emparelhamento...");
                    const code = await sock.requestPairingCode(NUMERO_TELEFONE);
                    console.log("\n========================================");
                    console.log(`🔑 NOVO CÓDIGO DE EMPARELHAMENTO: ${code}`);
                    console.log("========================================\n");
                } catch (err) {
                    console.error("Erro ao gerar código:", err);
                }
            }, 5000); // 5 segundos para garantir estabilidade
        }
